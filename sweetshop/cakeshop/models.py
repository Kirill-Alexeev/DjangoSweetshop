...
class Ingredient(models.Model):
    ...
    units = models.CharField(
        ...
        max_length=10,
        help_text="Наименование единиц измерения",
    )
    count = models.IntegerField(
        verbose_name="Количество", help_text="Количество ингредиента"
    )
...
class Cake(models.Model):
    ...
    description = models.TextField(
        verbose_name="Описание", max_length=1000, help_text="Описание торта"
    )
    image = models.CharField(
        verbose_name="Изображение", max_length=100, help_text="Изображение торта"
    )
    price = models.DecimalField(
        verbose_name="Цена",
        max_digits=10,
        decimal_places=2,
        help_text="Цена торта в руб.",
    )
...
class Review(models.Model):
    id = models.IntegerField(
        primary_key=True, auto_created=True, help_text="Уникальный ID для отзыва"
    )
    ...
    cake = models.ForeignKey(
        ...
        on_delete=models.CASCADE,
        verbose_name="Торт",
        help_text="Торт, на который оставили отзыв",
    )
...
class Order(models.Model):
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, verbose_name="Пользователь", help_text=""
    )
    execution_date = models.DateField(
        verbose_name="Дата доставки", help_text="Дата доставки заказа клиенту"
    )
    status = models.CharField(
        verbose_name="Статус",
        max_length=1,
        choices=[("О", "Ожидается"), ("З", "Завершён")],
    ...
    cakes = models.ManyToManyField(
        ...
        verbose_name="Торты",
        through="OrderCake",
        help_text="Торты добавленные в заказ",
    )
    delivery_address = models.CharField(
        verbose_name="Адрес доставки",
        max_length=255,
        help_text="Адрес, по которому доставляется заказ",
    )
    created_at = models.DateField(verbose_name="Дата создания", auto_now_add=True)
    updated_at = models.DateField(verbose_name="Дата обновления", auto_now=True)
...
class Cart(models.Model):
    user = models.OneToOneField(
        ...
        on_delete=models.CASCADE,
        verbose_name="Пользователь",
        help_text="Пользователь, владеющий корзиной",
    )
...
class CartItem(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.quantity} x {self.cake.title} в корзине {self.cart.user.username}"
        )
